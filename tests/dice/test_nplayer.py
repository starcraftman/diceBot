...
def test_yotube_dl(f_songs):
    try:
        ...
        dice.nplayer.youtube_dl(f_songs[0].url, 'song', tdir.name)
    ...
    try:
        ...
        for num in range(5):
            ...
            with open(tfile, 'w') as fout:
                ...
                now += 1
        dice.nplayer.prune_cache(tdir.name, limit=2 * 1024 ** 2)
...
def test_prune_cache_prefix():
    try:
        ...
        dice.nplayer.prune_cache(tdir.name, prefix='yt', limit=2 * 1024 ** 2)
...
def test_make_stream(f_songs):
    try:
        tdir = tempfile.TemporaryDirectory()
        stream = dice.nplayer.make_stream(f_songs[0])
...
def test_guild_player__init__(f_songs):
    player = dice.nplayer.GuildPlayer(vids=f_songs)
    assert isinstance(player, dice.nplayer.GuildPlayer)
    assert player.vids == f_songs


def test_guild_player__getattr__(f_songs, f_vclient):
    player = dice.nplayer.GuildPlayer(vids=f_songs, client=f_vclient)
...
@mock.patch('dice.nplayer.make_stream', lambda x: x)
def test_guild_player_play(f_songs, f_vclient):
    player = dice.nplayer.GuildPlayer(vids=[], client=f_vclient)
    player.vid_index = 2
...
def test_guild_player_play_no_vids(f_songs, f_vclient):
    player = dice.nplayer.GuildPlayer(vids=[])
    with pytest.raises(dice.exc.InvalidCommandArgs):
        player.play([])
